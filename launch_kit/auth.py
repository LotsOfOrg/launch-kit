...
# %% auto 0
__all__ = ['User', 'hash_password', 'verify_password', 'init_auth_tables', 'create_user', 'get_user_by_id', 'get_user_by_email',
...
class User:
    ...
    A simple dataclass that represents a user in the system.
    Compatible with FastHTML's MiniDataAPI for transparent database operations.
    """
    username: str
    email: str
    password_hash: str
...
def init_auth_tables(db_path: str = 'data.db') -> Database:
    """Initialize authentication database tables.
    
    Creates the users table with proper schema and indexes.
    Uses FastHTML's MiniDataAPI for simple, transparent database operations.
    
    Args:
        db_path: Path to SQLite database file (default: 'data.db')
        
    Returns:
        Database instance configured with User table
    """
...
def create_user(db: Database, username: str, email: str, password: str, 
                role: str = 'user', table_name: str = 'user') -> Optional[Dict[str, Any]]:
    ...
    Args:
        ...
        table_name: Table name (default: 'user')
        
    Returns:
        Created User dict or None if user exists
    """
    users = db[table_name]
...
def get_user_by_id(db: Database, user_id: int, table_name: str = 'user') -> Optional[Dict[str, Any]]:
    """Get user by ID.
...
def get_user_by_email(db: Database, email: str, table_name: str = 'user') -> Optional[Dict[str, Any]]:
...
def get_user_by_username(db: Database, username: str, table_name: str = 'user') -> Optional[Dict[str, Any]]:
...
def update_user(db: Database, user_id: int, table_name: str = 'user', **kwargs) -> Optional[Dict[str, Any]]:
    ...
    Args:
        ...
        table_name: Table name (default: 'user')
        **kwargs: Fields to update (e.g., email='new@email.com')
...
def delete_user(db: Database, user_id: int, table_name: str = 'user') -> bool:
...
def authenticate_user(db: Database, username_or_email: str, password: str, 
                     table_name: str = 'user') -> Optional[Dict[str, Any]]:
    ...
    Returns:
        Authenticated User dict or None if invalid credentials
    """
...
def check_permission(user: User, required_role: str = 'admin') -> bool:
    """Check if user has required role/permission.
    
    Simple role-based permission check. Can be extended for more complex permissions.
    
    Args:
        user: User object to check
        required_role: Required role (default: 'admin')
...
def track_login(db: Database, user_id: int, ip_address: Optional[str] = None, 
                user_agent: Optional[str] = None, table_name: str = 'user_logins') -> None:
    """Track user login for security and analytics.
    
    Creates a login record with timestamp and metadata.
    ...
    Args:
        ...
        table_name: Table name for login records (default: 'user_logins')
    """
...
def is_username_available(db: Database, username: str, table_name: str = 'user') -> bool:
...
def is_email_available(db: Database, email: str, table_name: str = 'user') -> bool:
...
def validate_user_data(username: str, email: str, password: str) -> List[str]:
