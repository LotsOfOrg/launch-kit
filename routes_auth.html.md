...
>  login_route (rt, path='/auth/login', redirect_to='/', login_form=None,
...
(default: ‘/auth/login’) redirect_to: Where to redirect after successful
...
<a
href="https://github.com/LotsOfOrg/launch-kit/blob/main/launch_kit/routes/auth.py#L104"
target="_blank" style="float:right; font-size:smaller">source</a>

### signup_route

>  signup_route (rt, path='/auth/signup', redirect_to='/', signup_form=None,
...
Args: rt: FastHTML router instance (from fast_app()) path: Route path
(default: ‘/auth/signup’) redirect_to: Where to redirect after
successful signup (default: ‘/’) signup_form: Custom form component
(callable returning FT) create_user: Custom user creation
function(form_data) -\> user_dict or error_string session_key: Session
key for auth data (default: ‘auth’) \*\*kwargs: Additional arguments
passed to form component\*
...
<a
href="https://github.com/LotsOfOrg/launch-kit/blob/main/launch_kit/routes/auth.py#L224"
target="_blank" style="float:right; font-size:smaller">source</a>

### logout_route

>  logout_route (rt, path='/auth/logout', redirect_to='/auth/login',
...
Args: rt: FastHTML router instance (from fast_app()) path: Route path
(default: ‘/auth/logout’) redirect_to: Where to redirect after logout
(default: ‘/auth/login’) session_key: Session key to clear (default:
‘auth’) before_logout: Optional callback function(session) called before
logout\*
